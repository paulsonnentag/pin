...
import { forEachClass, hasMethodNames, injectIntoConstructor, injectIntoMethod } from "../ast-helpers";
...
export const MAPLIBRE_MOD: LibraryMod = {
  ...
  mutate: (ast) => {
    forEachClass(ast, (classNode) => {
      ...
      if (hasMethodNames(classNode, ["addControl", "removeControl", "addSource", "addLayer"])) {
      ...
      if (hasMethodNames(classNode, ["setLngLat", "addTo", "remove"])) {
        ...
        injectIntoMethod(classNode, "setLngLat", function (self: any, lngLatLike: any) {
          let lng, lat;
